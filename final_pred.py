...
class Application:
    ...
    def __init__(self):
        ...
        self.root = tk.Tk()
        self.root.title("ASL to Text with Expression and Translation")
        self.root.protocol('WM_DELETE_WINDOW', self.destructor)
        self.root.geometry("1200x800")  #1300 x700
        ...
        self.T.config(text="ASL to Text with Expression and Translation", font=("Courier", 30, "bold"))
        ...
        self.speak.place(x=1005, y=630)
        self.speak.config(text="Speak", font=("Courier", 14), wraplength=100, command=self.speak_fun)

        self.clear = tk.Button(self.root)
        self.clear.place(x=925, y=630)
        self.clear.config(text="Clear", font=("Courier", 14), wraplength=100, command=self.clear_fun)

        self.clear = tk.Button(self.root)
        self.clear.place(x=805, y=630)
        self.clear.config(text="Translate", font=("Courier", 14), wraplength=120, command=self.clear_fun)
    ...
    def video_loop(self):
        try:
            ...
            hands,ii = hd.findHands(cv2image, draw=False, flipType=True)
            ...
            if hands:
                ...
                white = cv2.imread("C:\\Users\\nandi\\OneDrive\\Documents\\ASL\\ASLCode\\Devansh47\\white.jpg")
                # img_final=img_final1=img_final2=0

                handz, iii = hd2.findHands(image, draw=False, flipType=True)
                print(" ", self.ccc)
                self.ccc += 1
                if handz:
                    hand = handz[0]
